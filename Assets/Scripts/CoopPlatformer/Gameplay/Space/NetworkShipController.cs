...
using CoopPlatformer.Gameplay.Player;
using Unity.Netcode;
...
namespace CoopPlatformer.Gameplay.Space
{
    ...
    {
        private static readonly HashSet<NetworkShipController> ActiveShips = new();
        ...
        private void Update()
        {
            ...
            var input = _localInput.Read(_cam, transform.position, transform.up);
            var move = input.Move;
            var aim = input.Aim;
            var firing = input.Fire;
            ...
            if (_cam != null && _cam.TryGetComponent<CameraFollow>(out var follow))
        ...
        private void SetupCamera()
        {
            ...
            if (_cam != null)
            {
                if (!_cam.TryGetComponent<CameraFollow>(out var follow))
                    follow = _cam.gameObject.AddComponent<CameraFollow>();
                follow.SetTarget(transform);
            }
        }
        ...
        {
            var explosion = Instantiate(_config.CollisionExplosionPrefab, hitPosition, Quaternion.identity);
        ...
        {
            if (radius > 0f) _arenaRadius = radius;
        }
        ...
        private bool ShouldSendInput(Vector2 move, Vector2 aim, bool firing)
        {
            ...
            if (Time.unscaledTime >= _nextInputSendTime) return true;

            if (move != _lastSentMove) return Vector2.Distance(move, _lastSentMove) > _config.MoveSendThreshold;
