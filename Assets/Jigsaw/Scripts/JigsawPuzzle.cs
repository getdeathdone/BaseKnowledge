...
namespace Jigsaw.Scripts
{
    ...
    public class JigsawPuzzle : MonoBehaviour
    {
        ...
        public Texture
            image; // will contain the jigsaw projected picture                                 包含拼图的图片                						

        public Vector2 size = new(5, 5); // how many pieces will this puzzle have (x,y)								 size piece的总数量	

        public string
            topLeftPiece =
                "11"; // topleft piece - format YX (1,2,3,4,5) so 11 to 55 - 25 unique start possiblities       //???
        ...
        public int
            placePrecision =
                12; // how precise精度 must a piece beeing placed on the puzzle ( = percentage | 6 - 15 are good values , maybe higher up to 25 for small children )

        public bool
            scatterPieces =
                true; // scatter pieces when puzzle starts - if false the pieces are placed on their spot on the puzzle    //拼图散落开关
        ...
        private bool rotatePieces = false; // true to add rotation support - NOT IMPLEMENTED YET
                                            ...
                                            if (hits[h].collider.gameObject == puzzlePlane)
                                                    ...
                                                    activePiece == null)
                                                {
                                                    ...
                                                    if (_networkSync != null && _networkSync.IsPieceLocked(candidate.name))
                                                        // Piece is locked by someone else, skip it
                                                        continue;
                                ...
                                else
                                {
                                    ...
                                    if (neighborChain != null && outsideSnapping)
                                    {
                                        ...
                                        foreach (var piece in neighborChain)
                                        {
                                            ...
                                            chain =
                                                new List<GameObject>(); //we create a new list so we don't edit connectPieces[piece]
